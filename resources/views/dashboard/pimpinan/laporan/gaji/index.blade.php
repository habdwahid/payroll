...
<div class="card shadow mb-4">
    <div class="card-header">
        <h6 class="m-0 fw-bold text-primary">
            <i class="fa-solid fa-fw fa-table me-1"></i>Data Gaji
        </h6>
    </div>
    <div class="card-body px-lg-4">
        <form action="{{ route('pimpinan.gaji.index') }}" method="get">
            @csrf
            ...
            <table class="table table-bordered">
                <thead>
                    <tr class="text-center align-middle">
                        ...
                        <th class="col-1" scope="col">Jabatan</th>
                        <th class="col-1" scope="col">Gaji Pokok</th>
                        <th class="col-2" scope="col">Tunjangan Makan</th>
                        <th class="col-3" scope="col">Tunjangan Transport</th>
                        <th class="col-2" scope="col">Potongan</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr class="text-center align-middle">
                        ...
                        <td>{{ $attendanceList->user->user_data->position->position }}</td>
                        <td class="text-end">{{ 'Rp.' . number_format($attendanceList->user->user_data->position->salary->salary) }}</td>
                        <td class="text-end">{{ 'Rp.' . number_format($attendanceList->user->user_data->position->salary->meal_allowance) }}</td>
                        <td class="text-end">{{ 'Rp.' . number_format($attendanceList->user->user_data->position->salary->transport_allowance) }}</td>
                        <td class="text-end">
                            @php
                            $potongan = $attendanceList->absent * $absent->salary_cuts + $attendanceList->has_permission * $permission->salary_cuts + $attendanceList->sick * $sick->salary_cuts;
                            echo 'Rp.' . number_format($potongan)
                            @endphp
                        </td>
                    </tr>
                    @empty
...
<script>
    $(document).ready(function() {
        if ($("#month").val() == "") {
            $('a[title="Cetak"]').attr("href", "{{ url('laporan/gaji') }}/" + "{{ date('m') }}/" + $("#year").val());
        } else {
            $('a[title="Cetak"]').attr("href", "{{ url('laporan/gaji') }}/" + $("#month").val() + "/" + $("#year").val());
        }
    });
</script>
@endsection